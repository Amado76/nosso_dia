...
public record PatchRoutineItemRequest(UUID familyMemberId,
    ...
    public static PatchRoutineItemRequest fromJson(Map<String, Object> values) {
        JsonFields.only(values, "familyMemberId", "title", "description", "scheduledTime", "sortOrder", "tagIds");
        return new PatchRoutineItemRequest(JsonFields.uuid(values.get("familyMemberId")), JsonFields.string(values.get("title")), JsonFields.string(values.get("description")), JsonFields.time(values.get("scheduledTime")), JsonFields.integer(values.get("sortOrder")), JsonFields.uuidList(values,"tagIds"), values.keySet());
    }
}

...
public interface MediaRepository extends JpaRepository<Media, UUID> {
    ...
    @Query("select m from Media m where m.familyId = :familyId and (:unattached = false or not exists " +
            "(select l.id from PhotoRecordMedia l where l.mediaId = m.id) and not exists (select b.id from Book b where b.coverMediaId = m.id)) order by m.createdAt desc, m.id desc")

package com.nossodia.routine.entity;
...
@Entity
@Table(name = "routine_items", schema = "nosso_dia")
public class RoutineItem {
    @Id private UUID id;
    @Column(nullable = false) private UUID routineId;
    ...
    public RoutineItem(UUID routineId, UUID familyId, UUID familyMemberId, String title, String description, LocalTime time, Integer order, Instant now) {
        id = UUID.randomUUID(); this.routineId = routineId; this.familyId = familyId; this.familyMemberId = familyMemberId;
    ...
    public void assign(UUID memberId, Instant now) {
        JsonFields.required(memberId);
        if (!memberId.equals(familyMemberId)) { familyMemberId = memberId; updatedAt = now; }
    }
}

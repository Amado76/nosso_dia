...
class PlanningTests {
    ...
    UUID user() {
        ...
        jdbc.update("insert into nosso_dia.users(id,name,email,created_at,updated_at) values (?, 'Planner', ?, now(), now())", id, id + "@example.com");
        return id;
    }
    ...
    @Test void resolvesApplicableActiveItemsAndKeepsReadsSideEffectFree() throws Exception {
        ...
        assertThat(jdbc.queryForObject("select count(*) from nosso_dia.daily_plans where family_member_id = ?", Integer.class, UUID.fromString(m))).isZero();
    ...
    @Test void scopesDailyManagementReadsAndDoesNotCreateEmptyPlans() throws Exception {
        ...
        assertThat(jdbc.queryForObject("select count(*) from nosso_dia.daily_plans where family_member_id = ?",
    ...
    @Test void doesNotPersistEmptyNotesAndConvergesConcurrentFirstWrites() throws Exception {
        ...
        assertThat(jdbc.queryForObject("select count(*) from nosso_dia.daily_plans where family_member_id = ?", Integer.class, UUID.fromString(m))).isEqualTo(1);
    ...
    @Test void databaseEnforcesFamilyReferencesNonemptyDaysAndUniquePlans() throws Exception {
        ...
        assertThat(org.assertj.core.api.Assertions.catchThrowable(() ->
            jdbc.update("insert into nosso_dia.routine_items(id,family_id,routine_id,family_member_id,title,sort_order,created_at,updated_at) values (?, ?, ?, ?, 'Bad', 0, now(), now())", UUID.randomUUID(), fid, rid, UUID.fromString(otherM))))
                .isInstanceOf(org.springframework.dao.DataIntegrityViolationException.class);
        assertThat(org.assertj.core.api.Assertions.catchThrowable(() ->
            jdbc.update("delete from nosso_dia.routine_days where routine_id = ?", rid)))
                .isInstanceOf(org.springframework.dao.DataIntegrityViolationException.class);
        String day = f + "/members/" + m + "/daily-plan/2026-09-21";
        dailyItem(u, day, 0);
        assertThat(org.assertj.core.api.Assertions.catchThrowable(() ->
            jdbc.update("insert into nosso_dia.daily_plans(id,family_id,family_member_id,plan_date,created_at,updated_at) values (?, ?, ?, '2026-09-21', now(), now())", UUID.randomUUID(), fid, UUID.fromString(m))))
                .isInstanceOf(org.springframework.dao.DataIntegrityViolationException.class);
        UUID plan = jdbc.queryForObject("select id from nosso_dia.daily_plans where family_member_id = ?", UUID.class, UUID.fromString(m));
        assertThat(org.assertj.core.api.Assertions.catchThrowable(() ->
            jdbc.update("insert into nosso_dia.daily_plan_items(id,daily_plan_id,title,sort_order,created_at,updated_at) values (?, ?, 'Bad', 0, now(), now())", UUID.randomUUID(), plan)))
                .isInstanceOf(org.springframework.dao.DataIntegrityViolationException.class);
    }
}

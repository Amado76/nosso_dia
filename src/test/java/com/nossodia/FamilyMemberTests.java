...
class FamilyMemberTests {
    ...
    UUID user() {
        ...
        jdbc.update("insert into nosso_dia.users (id, name, email, created_at, updated_at) values (?, 'Test User', ?, now(), now())", id, id + "@example.com");
        return id;
    }
    ...
    void createsNormalizedPeopleWithOptionalFieldsAndPrivateResponse() throws Exception {
        ...
        member(owner, family);
        member(owner, family);
        assertThat(jdbc.queryForObject("select count(*) from nosso_dia.family_members where family_id = ?", Integer.class, family)).isEqualTo(3);
    }
    ...
    void rejectsInvalidCreationWithoutPersisting() throws Exception {
        ...
        assertThat(jdbc.queryForObject("select count(*) from nosso_dia.family_members where family_id = ?", Integer.class, family)).isZero();
    }
    ...
    void filtersOrdersAndReadsInactivePeople() throws Exception {
        ...
        jdbc.update("update nosso_dia.family_members set created_at = '2026-01-01T00:00:00Z' where family_id = ?", family);
    ...
    void linksAndUnlinksOwnAccountForEveryRoleWithIdempotencyAndNoAccessGrant() throws Exception {
        ...
        assertThat(jdbc.queryForObject("select count(*) from nosso_dia.family_memberships where family_id = ?", Integer.class, family)).isEqualTo(3);
    }
    ...
    void databaseEnforcesLinksMembershipAndStructuralConstraints() throws Exception {
        ...
        for (String assignment : new String[]{"name = '   '", "name = E'\\t\\n'", "member_type = 'OTHER'", "color = '#abcdef'", "color = '#12345G'", "family_id = '" + UUID.randomUUID() + "'"}) {
            org.assertj.core.api.Assertions.assertThatThrownBy(() -> jdbc.update("update nosso_dia.family_members set " + assignment + " where id = ?", first))
                    .isInstanceOf(org.springframework.dao.DataIntegrityViolationException.class);
        }
        org.assertj.core.api.Assertions.assertThatThrownBy(() -> jdbc.update("update nosso_dia.family_members set linked_user_id = ? where id = ?", outsider, first))
                .isInstanceOf(org.springframework.dao.DataIntegrityViolationException.class);
        jdbc.update("update nosso_dia.family_members set linked_user_id = ? where id = ?", owner, first);
        org.assertj.core.api.Assertions.assertThatThrownBy(() -> jdbc.update("update nosso_dia.family_members set linked_user_id = ? where id = ?", owner, second))
                .isInstanceOf(org.springframework.dao.DataIntegrityViolationException.class);
        org.assertj.core.api.Assertions.assertThatThrownBy(() -> jdbc.update("delete from nosso_dia.family_memberships where family_id = ? and user_id = ?", family, owner))
                .isInstanceOf(org.springframework.dao.DataIntegrityViolationException.class);
        jdbc.update("update nosso_dia.family_members set linked_user_id = null where id = ?", first);
        jdbc.update("delete from nosso_dia.family_memberships where family_id = ? and user_id = ?", family, owner);
        request(owner, get(path(family))).andExpect(status().isNotFound());
        assertThat(jdbc.queryForObject("select count(*) from nosso_dia.family_members where family_id = ?", Integer.class, family)).isEqualTo(2);
    }

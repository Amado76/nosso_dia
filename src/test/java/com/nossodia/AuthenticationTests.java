...
class AuthenticationTests {
    ...
    @Autowired com.nossodia.auth.service.ExternalIdentityService externalIdentities;
    @org.springframework.test.context.bean.override.mockito.MockitoBean
    com.nossodia.auth.mail.PasswordResetDelivery delivery;
    ...
    void normalizesEmailEnforcesUniquenessAndHashesPassword() throws Exception {
        ...
        String hash = jdbc.queryForObject("select password_hash from nosso_dia.users where email = ?", String.class, email);
    ...
    void rejectsMissingInvalidAndExpiredAccessTokensAndDefaultsToProtected() throws Exception {
        ...
        var claims = org.springframework.security.oauth2.jwt.JwtClaimsSet.builder().issuer("nosso-dia")
                .subject(UUID.randomUUID().toString()).audience(java.util.List.of("nosso-dia-api"))
    ...
    void expiredRefreshIsRejectedAndOnlyHashesAreStored() throws Exception {
        ...
        String stored = jdbc.queryForObject("select token_hash from nosso_dia.refresh_tokens where user_id = ?", String.class, userId);
        org.assertj.core.api.Assertions.assertThat(stored).hasSize(64).isNotEqualTo(raw);
        jdbc.update("update nosso_dia.refresh_tokens set created_at = now() - interval '2 days', expires_at = now() - interval '1 day' where user_id = ?", userId);
    ...
    void passwordResetIsSingleUseChangesPasswordAndRevokesAllSessions() throws Exception {
        ...
        String hash = jdbc.queryForObject("select token_hash from nosso_dia.password_reset_tokens where user_id = ?", String.class, id);
    ...
    void rejectsExpiredAndUnknownPasswordResetTokens() throws Exception {
        ...
        var captured = org.mockito.ArgumentCaptor.forClass(String.class);
        org.mockito.Mockito.verify(delivery).send(org.mockito.ArgumentMatchers.eq(email), captured.capture());
        jdbc.update("update nosso_dia.password_reset_tokens set created_at = now() - interval '2 days', expires_at = now() - interval '1 day' where user_id = ?", id);
    ...
    void storesUniqueProviderSubjectsIndependentlyOfProviderEmail() throws Exception {
        ...
        jdbc.update("insert into nosso_dia.external_identities(id,user_id,provider,provider_subject,created_at) values (?,?,?,?,now())", UUID.randomUUID(), id, "GOOGLE", subject);
        org.assertj.core.api.Assertions.assertThatThrownBy(() -> jdbc.update(
                "insert into nosso_dia.external_identities(id,user_id,provider,provider_subject,created_at) values (?,?,?,?,now())", UUID.randomUUID(), id, "GOOGLE", subject))
                .isInstanceOf(org.springframework.dao.DataIntegrityViolationException.class);
    }

    @org.junit.jupiter.params.ParameterizedTest
    @org.junit.jupiter.params.provider.EnumSource(com.nossodia.auth.entity.ExternalProvider.class)
    void resolvesVerifiedProviderSubjectsWithoutCreatingOrMergingAccounts(com.nossodia.auth.entity.ExternalProvider provider) throws Exception {
        String email = UUID.randomUUID() + "@example.com";
        UUID id = UUID.fromString(field(register(email), "id"));
        String subject = UUID.randomUUID().toString();
        jdbc.update("insert into nosso_dia.external_identities(id,user_id,provider,provider_subject,created_at) values (?,?,?,?,now())",
                UUID.randomUUID(), id, provider.name(), subject);
        String issuer = provider == com.nossodia.auth.entity.ExternalProvider.GOOGLE ? "https://accounts.google.com" : "https://appleid.apple.com";
        var identity = oidcIdentity(issuer, subject, "different@example.com");
        int before = jdbc.queryForObject("select count(*) from nosso_dia.users", Integer.class);
        org.assertj.core.api.Assertions.assertThat(externalIdentities.resolve(provider, identity).id()).isEqualTo(id);
        org.assertj.core.api.Assertions.assertThat(externalIdentities.resolve(provider, identity).id()).isEqualTo(id);
        org.assertj.core.api.Assertions.assertThat(jdbc.queryForObject("select count(*) from nosso_dia.users", Integer.class)).isEqualTo(before);
        org.assertj.core.api.Assertions.assertThatThrownBy(() -> externalIdentities.resolve(provider, oidcIdentity(issuer, "unlinked", email)))
                .isInstanceOf(com.nossodia.shared.exception.ApiException.class)
                .extracting(e -> ((com.nossodia.shared.exception.ApiException) e).getCode()).isEqualTo("EXTERNAL_IDENTITY_NOT_LINKED");
    ...
    void concurrentRegistrationReturnsAStableConflictWithoutLoggingEmail(org.springframework.boot.test.system.CapturedOutput output) throws Exception {
        ...
        org.assertj.core.api.Assertions.assertThat(jdbc.queryForObject("select count(*) from nosso_dia.users where email = ?", Integer.class, email)).isEqualTo(1);
    }
